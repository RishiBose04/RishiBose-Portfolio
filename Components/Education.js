...
const Education = ({ currentTheme }) => {
    return (
        <div className={styles.educationWrapper}>
            ...
            <div style={{ textAlign: 'center', paddingBottom: '2rem' }}><Link
                href="/#about"
                className={styles.cta3}
                style={{ background: currentTheme.accent, color: currentTheme.contrastText }}>{ctaTexts.educationCTA} <span>&gt;</span></Link></div>
        </div>
    );
}
